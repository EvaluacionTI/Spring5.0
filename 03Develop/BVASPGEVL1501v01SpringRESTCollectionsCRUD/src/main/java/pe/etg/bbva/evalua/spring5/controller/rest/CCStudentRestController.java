...
public class CCStudentRestController {
	private static final Logger MOLOG = LoggerFactory.getLogger(CCStudentRestController.class);
	
	private CRStudentRepository oCRrepository;
	
	/*
	 * 1. INYECTAR EL REPOSITORIO : Esta se realiza en modo constructor
	 */
	@Autowired
	public CCStudentRestController(CRStudentRepository poCRrepository) {
		
		MOLOG.info("[EVL]  CRStudentRepository : {} ", poCRrepository);
	...
	*     Para manejar casos donde no se encuentra el estudiante
	...
	public Collection<CEStudent> readStudents(){
		...
		MOLOG.info("[EVL]  Collection<CEStudent> : {} ", oCRrepository);
...
//	public CEStudent readStudent(@PathVariable Long pId) { -- Con esto se visualiza un error del tipo de dato
	public CEStudent readStudent(@PathVariable("id") Long pId) {
		MOLOG.info("[EVL]  @PathVariable Long pId) : {} ", pId);
	...
	ResponseEntity<?> addStudent(@RequestBody CEStudent student){
		MOLOG.info("[EVL] CEStudent : {} ", student.toString());
		...
		MOLOG.info("[EVL]  CEStudent POST: {} ", result);
		MOLOG.info("[EVL]  URI : {} ", location);
	...
	CEStudent updateStudent(@RequestBody CEStudent student) {
		MOLOG.info("[EVL]  @RequestBody CEStudent student : {} ", student);
	...
	void deleteStudent(@PathVariable Long id) {
		MOLOG.info("[EVL]  @PathVariable Long id : {} ", id);
