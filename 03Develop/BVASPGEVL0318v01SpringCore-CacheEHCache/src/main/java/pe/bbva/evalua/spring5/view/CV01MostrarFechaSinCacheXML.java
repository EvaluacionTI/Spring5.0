...
public class CV01MostrarFechaSinCacheXML {
	private static final Logger LOG = LoggerFactory.getLogger("CV01MostrarFechaSinCacheXML");
	...
	public static void main(String[] args) {
		...
		Thread t = new Thread(() -> {
			try {
				for (int i = 0; i < 10; i++) {
					Thread.sleep(1000);
					LOG.info("[EVL] Send cache : {}", oCServicio.fechaActualSinCache());
				}
			} catch (Exception ex) {
				ex.printStackTrace();
			}
