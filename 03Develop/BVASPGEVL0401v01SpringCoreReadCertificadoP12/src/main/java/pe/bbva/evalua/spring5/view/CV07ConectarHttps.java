...
public class CV07ConectarHttps {
	private static final Logger LOG = LoggerFactory.getLogger("CV07ConectarHttps");
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 4. Abrimos la conexión y le pasamos nuestro contexto SSL
			URL url = new URL(URL_DESTINO);
			LOG.info("[EVL] Instancia URL : {}", url);
			URLConnection conexion = url.openConnection();
			LOG.info("[EVL] Instancia URLConnection : {}", conexion);
			
			((HttpsURLConnection) conexion).setSSLSocketFactory(sslSocketFactory);
			LOG.info("[EVL] Instancia HttpsURLConnection : {}", conexion);
			
			//5. Conectarnos HttpURL
			conexion.connect();
